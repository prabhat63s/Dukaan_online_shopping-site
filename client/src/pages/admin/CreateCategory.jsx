...
export default function CreateCategory() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("https://dukaan-online-shopping-site.onrender.com/api/v1/category/create-category", {
        name,
      });
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://dukaan-online-shopping-site.onrender.com/api/v1/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data?.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `https://dukaan-online-shopping-site.onrender.com/api/v1/category/delete-category/${pId}`
      );
      if (data.success) {
